...
1. **신고번호**:
   ...
   - 신고번호가 없거나 읽을 수 없는 경우 "정보 없음"

2. **송품장번호**:
   - 위치: 송품장부호 (문서 상단 오른쪽에 위치, 신고번호 항목 근처)
   - 형태: 영문자+숫자 조합 또는 "-" 표시
   - 실제 문서에서는 신고번호 영역 근처에 작은 글씨로 표기되어 있음
   - 빈 칸이거나 없으면 "정보 없음"
...
9. **출발지**:
   - 위치: 적재항 (문서 중간 부분 왼쪽, 거래구분 항목 위쪽에 위치)
   - 형태: 영문 항구코드 또는 항구명 (예: KRINC, BUSAN)
   - 실제 문서에서는 "적재항" 라벨 다음에 영문으로 표기됨
   - 작은 글씨로 되어 있을 수 있음
   - 항구코드가 없는 경우 "정보 없음"

10. **도착지**:
    - 위치: 목적국 (문서 중간 부분 왼쪽, 거래구분 항목 바로 아래에 위치)
    - 형태: 영문 국가명 (예: VIETNAM, CAMBODIA)
