...
7. **추가 설명이나 주석 없이 순수한 JSON만 반환하세요**"""
    ...
    def _split_transfer_receipts(self, page_texts: List[Dict]) -> List[str]:
        ...
        for page in page_texts:
            ...
            if any(keyword in text.lower() for keyword in ['이체확인증', '송금증', '이체금액', '출금금액', '받는분']):
                individual_docs.append(text)
...
"""
        ...
        if doc_type == "수출신고필증":
            return base_prompt + """
이 수출신고필증(갑지)에서 **다음 11개 필드만** 정확히 추출해주세요.
...
4. 존재하지 않는 정보는 "정보 없음"으로 표시
5. **요청하지 않은 필드는 절대 추가하지 마세요**
6. **문서에 있는 다른 모든 정보는 무시하고 위 8개 필드만 추출하세요**
7. **추가 설명이나 주석 없이 순수한 JSON 배열만 반환하세요**
8. **각 필드명은 정확히 위에 명시된 대로 사용하세요**"""
